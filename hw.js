...
const allStartingWithA = function (words) {
  // Your Code Here
  for (let i = 0; i < words.length; i++) {
    if (words[i].toUpperCase().startsWith('A') === false) {
      return false;
    }
  }
...
const anyStartingWithB = function (words) {
  for (let i = 0; i < words.length; i++) {
    if (words[i].toUpperCase().startsWith('B') === true) {
      return true;
    }
  }
  return false;
...
const wordsWithAtLeastTwoVowels = function (words) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (sum >= 2) {
      wordsWithTwoVowels.push(words[i]);
    }
